...
public class PlayerSearchFragment extends BaseFragment {
   ...
   * @param savedInstanceState Previously saved state that is the fragment is re-constructed from if
   *     not null
  ...
  private void setUpSearchFragment(View view) {
    ...
    searchQueryList.setOnItemClickListener(
        (parent, v, position, id) -> {
          SearchQueryEntry entry = searchQueryEntries.get(position);
          controller.handleSearchQueryListClick(entry);
        });

    controller.displaySearchQueryData(
        searchQuery, searchQueryEntries, entryAdapter, searchLinearLayout, getContext());
  }
}
